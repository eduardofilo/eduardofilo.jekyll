...
Es útil aprovechar la funcionalidad del sensor de distancia para disparar el comportamiento de Otto que queremos programar. En otro caso todo lo que programemos dentro de la función `loop` se ejecutará sin parar. Así por ejemplo, imitando la función `obstacleMode`, un ejemplo de `loop` en el que haremos que Otto avance un par de pasos, haga el moonwalk hacia la derecha, dé un pequeño salto y se detenga ejecutando el gesto de victoria sería:
