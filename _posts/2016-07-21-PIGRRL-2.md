...
1. Bajamos Rasbien Lite: https://www.raspberrypi.org/downloads/raspbian/
2. Instalamos:
...
La PiTFT de Adafruit funciona haciendo framebuffer de la memoria de vídeo de la Raspberry Pi hacia la PiTFT por SPI (explicado [aquí](https://github.com/notro/fbtft/wiki/Framebuffer-use#framebuffer-mirroring)). Para que esto funcione hay que instalar un pequeño programa.
...
2. Editamos el fichero `~/Adafruit-Retrogame/retrogame.c` y sustituimos la estructura `ioStandard` que hay a partir de la línea 111 por lo que sigue. Esto hará corresponder algunas de las teclas importantes para manejar los emuladores y el propio EmuleStation con el mapeo de pulsadores que hay hacia el GPIO de la Raspberry en el PiGRRL 2:
...
4. Crear el fichero `/etc/udev/rules.d/10-retrogame.rules` con el siguiente contenido:

        SUBSYSTEM=="input", ATTRS{name}=="retrogame", ENV{ID_INPUT_KEYBOARD}="1"

5. Añadir lo siguiente al final de `/etc/rc.local`:

        /usr/local/bin/retrogame &

6. Añadir los siguientes módulos al fichero `/etc/modules`:

        uinput
        joydev
        snd-bcm2835

7. Sustituir el contenido del fichero `/opt/retropie/configs/all/emulationstation/es_input.cfg` por:
